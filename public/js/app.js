        ...
        file.addEventListener('change', (event) => {
            if (event.target.files.length > 0) {
                ...
                if (preview) {
                    ...
                    /* }*/
                    // preview.src = src;
                    ['fr-fi-instagram-line', 'fr-py-7v', 'fr-fi-refresh-line'].map(v => event.target.parentElement.classList.toggle(v));
                    fileIsOk = true;
                    ...
                    if (event.target.files[0].size > 10 * 1024 * 1024) {
                       ...
                       resTextEl.classList.add('fr-hidden')
                        fileIsOk = true;
                        ['fr-fi-attachment-fill', 'fr-fi-refresh-line'].map(v => event.target.parentElement.classList.toggle(v));
                    }
                }
                if (fileIsOk) {
                    ...
                    deleter.addEventListeners('click touchdown', (e) => {
                        ...
                        if (preview) {
                            ...
                            ['fr-fi-attachment-fill'].map(v => event.target.parentElement.classList.toggle(v));
                        }
                        event.target.value = '';
                    ...
                    request.addEventListener('load', function (e) {
                        ...
                        event.target.parentElement.classList.toggle('fr-fi-refresh-line');
                        [preview, deleter].forEach(el => el?.classList?.toggle('fr-hidden'));
                        progress.value = 0;
                        let jsonRes = JSON.parse(request.response)
                        if (request.status !== 200) {
                            resTextEl.innerHTML = jsonRes.error;
                            resTextEl.classList.remove('fr-hidden')
                            progress.value = 0;
                            deleter.click()
                        } else {
                            resTextEl.innerHTML = jsonRes.file;
                            ['fr-text-label--green-emeraude', 'fr-text-label--red-marianne', 'fr-hidden'].map(c => resTextEl.classList.toggle(c));
                            if(!preview)
                                ['fr-fi-checkbox-circle-fill'].map(v => event.target.parentElement.classList.toggle(v));
                            localStorage.setItem(event.target.name,request.response)
                        }
                        progress.classList.add('fr-hidden');
                    });
                    progress.classList.remove('fr-hidden')
                    request.send(fileData);
                    //RODO: FINIR PREUPLOAD TEMP FOLDER
                }
            }
        })
    })
